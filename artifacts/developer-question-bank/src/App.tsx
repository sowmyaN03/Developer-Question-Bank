...
const subjectGroups: Record<string, string[]> = {
  ...
  'Java & Spring': ['Core Java', 'Spring', 'Spring Boot', 'Java Spring', 'Hibernate', 'JSP'],
...
function Dashboard({ attempts }: { attempts: Attempt[] }) {
  ...
  return <div className="animate-in">
    ...
    <section className="mt-7"><div className="mb-4 flex items-end justify-between"><div><h2 className="text-[18px] font-bold text-[#e1e9e7]">Subjects</h2><p className="mt-1 text-[11px] text-[#718192]">Choose a subject to open its topic-wise quiz sets.</p></div><span className="font-mono text-[10px] text-[#718394]">{seedQuestions.length} questions</span></div>{Object.entries(subjectGroups).map(([group, groupSubjects]) => <div key={group} className="mb-7"><div className="mb-3 flex items-center justify-between"><h3 className="text-[14px] font-bold text-[#dbe5e2]">{group}</h3><span className="font-mono text-[10px] text-[#718394]">{groupSubjects.reduce((sum, subject) => sum + seedQuestions.filter((q) => q.subject === subject).length, 0)} questions</span></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{groupSubjects.filter((subject) => subjects.includes(subject)).map((subject) => { const count = seedQuestions.filter((q) => q.subject === subject).length; return <Link key={subject} href={`/subject/${encodeURIComponent(subject)}`} className="group rounded-lg border border-[#24313e] bg-[#0d151e] p-4 transition-colors hover:border-[#397b6b] hover:bg-[#101c24]" data-testid={`link-grid-subject-${subject.toLowerCase().replaceAll(' ', '-')}`}><div className="mb-5 flex items-start justify-between"><span className="tag tag-teal">{subject}</span><ArrowRight size={14} className="text-[#617486] transition-transform group-hover:translate-x-1" /></div><div className="flex items-center gap-2 text-[10px] text-[#718394]"><span>{count} questions</span><span>·</span><span>{Math.max(1, Math.ceil(count / 30))} quiz sets</span></div></Link>; })}</div></div>)}</section>
...
function TopicSubjectPage({ onCreate }: { onCreate: (config: ExamConfig) => void }) {
  const params = useParams<{ subject: string }>();
  const subject = decodeURIComponent(params.subject || '');
  const items = seedQuestions.filter((question) => question.subject === subject);
  const byTopic = items.reduce<Record<string, Question[]>>((groups, question) => {
    (groups[question.topic] ||= []).push(question);
    return groups;
  }, {});
  const topics = Object.entries(byTopic);
  return <div className="animate-in">
    <Link href="/" className="mb-7 inline-flex items-center gap-2 text-[11px] font-bold text-[#91a1ad] hover:text-[#dce8e4]" data-testid="link-all-subjects"><ArrowLeft size={14} /> All subjects</Link>
    <section className="panel mb-7 p-5 sm:p-7"><div className="flex items-start gap-4"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-[#24544d] bg-[#112a27] text-[#62d8b6]"><BookOpen size={21} /></span><div><h1 className="text-[24px] font-extrabold tracking-[-.04em] text-[#e8f0ed] sm:text-[29px]">{subject || 'Subject'}</h1><p className="mt-2 text-[11px] text-[#7d8e9c]">{items.length} questions · {topics.length} topics · {Math.max(1, Math.ceil(items.length / 30))} generated sets</p></div></div></section>
    <div className="mb-3"><h2 className="text-[14px] font-bold text-[#e1e9e7]">Quiz Sets</h2><p className="mt-1 text-[11px] text-[#718192]">Each topic is kept together and split into sets of no more than 30 questions. Choose <span className="text-[#5ed7b4]">Practice</span> for explanations after each answer, or <span className="text-[#e06c78]">Exam</span> for results at the end.</p></div>
    <div className="space-y-2">{topics.flatMap(([topic, topicItems]) => Array.from({ length: Math.ceil(topicItems.length / 30) }, (_, setIndex) => ({ topic, setIndex, questions: topicItems.slice(setIndex * 30, setIndex * 30 + 30) }))).map(({ topic, setIndex, questions }) => <div key={`${topic}-${setIndex}`} className="panel p-4"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-[12px] font-bold text-[#dbe5e2]">{topic} <span className="ml-2 tag">{setIndex + 1} / {Math.ceil((byTopic[topic]?.length || 0) / 30)}</span></div><div className="mt-1 text-[10px] text-[#718394]">{questions.length} questions · topic-wise set</div></div><div className="flex gap-2"><button onClick={() => onCreate({ subjects: [subject], questionIds: questions.map((question) => question.id), questionCount: questions.length, difficulty: 'Mixed', mode: 'untimed' })} className="button-small" data-testid={`button-practice-topic-${setIndex}`}><Play size={12} /> Practice</button><button onClick={() => onCreate({ subjects: [subject], questionIds: questions.map((question) => question.id), questionCount: questions.length, difficulty: 'Mixed', mode: 'timed' })} className="button-primary" data-testid={`button-exam-topic-${setIndex}`}><Target size={12} /> Exam</button></div></div></div>)}</div>
    {!topics.length && <div className="panel"><EmptyState icon={BookOpen} title="Subject not found" description="Return to the subject index and choose another topic." action={<Link href="/" className="button-secondary">Back to overview</Link>} /></div>}
  </div>;
}
